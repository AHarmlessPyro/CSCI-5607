...
int lastMat = INT32_MIN;//std::numeric_limits<std::int32_t>::max();
...
void fillVars(vector<std::string> tokenizedString) {
	...
	// If first token var is vfov
		...
		if (fov < 0.0 || fov > 90.0) {
			const char* string = "Invalid FOV. %f is less than 0 or greater than 90.0\0";
			char* temp = (char*)calloc(EXCEPTIONSTRINGSIZE, sizeof(char));
			int val = snprintf(temp, EXCEPTIONSTRINGSIZE, string, fov);
		...
		if (width < 0 || height < 0) {
			...
			char* temp = (char*)calloc(EXCEPTIONSTRINGSIZE, sizeof(char));
			int val = snprintf(temp, EXCEPTIONSTRINGSIZE, string, width, height);
			std::cout<<temp<<std::endl;
			// throw vs exit issue.
			//throw std::out_of_range(temp);
			exit(EXIT_FAILURE);
		...
		if ((red < 0 || red >1.0) || (green < 0 || green >1.0) || (blue < 0 || blue >1.0)) {
			...
			char* temp = (char*)calloc(EXCEPTIONSTRINGSIZE, sizeof(char));
			int val = snprintf(temp, EXCEPTIONSTRINGSIZE, string, red, green, blue);
			std::cout<<temp<<std::endl;
			// throw vs exit issue
			// throw std::out_of_range(temp);
			exit(EXIT_FAILURE);
		...
		if (lastMat == std::numeric_limits<std::int32_t>::min()) {

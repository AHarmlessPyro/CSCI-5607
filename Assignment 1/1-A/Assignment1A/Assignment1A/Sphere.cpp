...
Object3D::Hit* Sphere::hit(Ray &r, float tmin, float tmax) {
  ...
  Object3D::Hit* temp = (Object3D::Hit*)calloc(1,sizeof(Object3D::Hit));
	...
	temp->hit = false;
	temp->tVal = -1.0;
	temp->point = Vector3(std::numeric_limits<float>::lowest(),std::numeric_limits<float>::lowest() ,std::numeric_limits<float>::lowest());
	temp->lastHit = NULL;
	return temp;
